...
def get_items_in_directory(
    ...
    if recursive:
        for folder in folders:
            children_folders, children_files = get_items_in_directory(
                site=site,
                source_folder_path=Path(folder.properties['ServerRelativeUrl']),
                recursive=recursive,
                )
            
            file_list.extend(children_files)
            folder_list.extend(children_folders)
            

    children_files = ctx_client.web.get_folder_by_server_relative_url(directory_relative_uri).files
    ctx_client.load(children_files)
    ctx_client.execute_query()
